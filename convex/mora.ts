...
/**
 * Create the default MoraSettings object.
 *
 * @returns A `MoraSettings` object with `enabled` set to `true`, `yoloMode` set to `false`, `allowWrites` set to `true`, `allowedWriteScopes` initialized from `DEFAULT_ALLOWED_SCOPES`, and `updatedAt` set to the current ISO timestamp.
 */
...
/**
 * Fetches the Mora settings document stored under the module's settings key.
 *
 * @returns The settings row for `MORA_SETTINGS_KEY`, or `null` if no document exists.
 */
...
/**
 * Resolves the current Mora settings by merging any stored settings over the defaults.
 *
 * @returns The effective `MoraSettings` where values from the stored settings (if present) override the default settings.
 */
...
/**
 * Returns the most recent baby profile id for a family the user belongs to, searching families in the order provided.
 *
 * @param userId - The id of the user whose families will be searched
 * @returns The id of the latest baby profile found for one of the user's families, or `null` if no baby profiles exist
 */
...
/**
 * Infer the Mora permission scope associated with an action type string.
 *
 * @param actionType - Action type identifier (for example `"event.create"`, `"reminder.update"`, or `"note.create"`)
 * @returns `'events'` if `actionType` starts with `"event."`, `'reminders'` if it starts with `"reminder."`, `'notes'` if it equals `"note.create"`, `'unknown'` otherwise.
 */
...
/**
 * Ensures the caller is authenticated and authorized to access a specific Mora action and its thread, then returns the authenticated user and the action record.
 *
 * @param actionId - The ID of the Mora action to validate
 * @returns An object containing the authenticated `user` and the `action` record
 * @throws If the action or its thread is not found
 * @throws If the thread is associated with a baby and the caller lacks access to that baby
 */
