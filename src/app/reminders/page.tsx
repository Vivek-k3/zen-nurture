...
/**
 * Render a list of selectable time inputs with add/remove controls and formatted display.
 *
 * Renders each time as a compact pill showing a localized "hour:minute AM/PM" label, an invisible native time input to edit the value, a remove button (disabled when only one time remains), and an "Add Time" button that appends a new entry. All changes are propagated via `onChange`.
 *
 * @param times - Array of time strings in "HH:MM" 24-hour format
 * @param onChange - Callback invoked with the updated `times` array whenever a time is added, removed, or edited
 * @returns The TimePickerList React element
 */
...
/**
 * Render the Reminders management page, including upcoming reminders, reminder rules, and the Add Reminder modal.
 *
 * Renders a header, an "Upcoming" list, a "Reminder Rules" list with enable/delete controls, and a slide-over panel to create new reminder rules (supports fixed times and after-last-event triggers). Data is fetched for the current baby profile and reminder rules; UI actions create, update, and delete rules via mutations.
 *
 * @returns A React element representing the Reminders page; returns `null` until the component has mounted.
 */
