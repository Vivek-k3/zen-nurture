...
/**
 * Render trend charts visualizing feeds, diapers, sleep, and medication over a date range.
 *
 * Builds a daily data series from `rangeAggregates` and renders chart sections for:
 * feed volume, feed count, diapers (wet/dirty/dry/mixed), sleep duration, and medication adherence.
 *
 * @param rangeAggregates - Optional mapping keyed by `YYYY-MM-DD` to aggregate objects. Expected aggregate fields used: `feeds.totalMl`, `feeds.count`, `diapers.count`, `diapers.wet`, `diapers.dirty`, `diapers.dry`, `diapers.mixed`, `sleeps.totalMin`, `meds.taken`, and `meds.skipped`.
 * @param days - Number of days to include in the generated series (most recent `days` ending today).
 * @returns A React element containing the assembled charts, or a no-data placeholder when `rangeAggregates` is falsy or yields no rows.
 */
...
/**
 * Renders a chart tooltip showing the date, total diaper count, and a breakdown of wet/dirty/dry/mixed diapers.
 *
 * @param active - Whether the tooltip is active (visible).
 * @param payload - The Recharts payload array; the component reads the first payload's `payload` row for data.
 * @returns A JSX element with the tooltip content, or `null` when inactive or when `payload` is empty.
 */
