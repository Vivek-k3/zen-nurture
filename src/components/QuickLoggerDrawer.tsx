...
/**
 * Renders a clickable log option tile with an icon, label, and color accent.
 *
 * @param icon - Material symbol name to display inside the tile's icon circle
 * @param label - Visible text label shown below the icon
 * @param color - Tailwind color token used for the tile accent (e.g. "blue-500")
 * @param onClick - Click handler invoked when the tile is pressed
 * @returns The React element for the log option tile
 */
...
/**
 * Render a volume picker card for selecting an amount in milliliters.
 *
 * @param value - Current volume value in milliliters.
 * @param onChange - Callback invoked with the new volume (ml) when the user changes the value via input, slider, or presets.
 * @param presets - Array of preset volume values (in ml) rendered as quick-select buttons.
 * @param sliderClassName - Optional className applied to the range slider element.
 * @param presetClassName - Optional className applied to each preset button.
 * @returns The rendered volume picker card element.
 */
 ...
 *
 * @returns A Date representing the current time with seconds and milliseconds set to zero.
 */
...
/**
 * Produce a Date for the time a given number of minutes before now, normalized to whole-minute precision.
 *
 * @param minutes - Number of minutes in the past
 * @returns A Date object representing (now - `minutes`) with seconds and milliseconds cleared
 */
...
/**
 * Rounds a Date down to the start of its minute by zeroing seconds and milliseconds.
 *
 * @returns A new `Date` equal to the input with seconds and milliseconds set to `0`.
 */
...
/**
 * Produce a concise, human-facing label for the event time picker based on the current log view and feed subtype.
 *
 * @param view - The current log view (e.g., "feed", "diaper", "meds", "note", "growth").
 * @param feedSubType - The feed subtype when `view` is "feed" (e.g., "bottle", "breast", "pump").
 * @returns A short label string such as "Pump Started", "Feed Started", "Diaper Changed", "Medicine Given", "Note Added", "Measurement Taken", or "Event Time".
 */
